// konstanta ethers yang akan membutuhkan variable ethers
const ethers = require("ethers");
// const solc = require("solc")
// konstanta fs yang akan membutuhkan variable fs-extra
const fs = require("fs-extra");
// meingimport konfigurasi dotenv
require("dotenv").config();

// fungsi asinkron dari kode ini
async function main() {
  // First, compile this!
  // pertama-tama kita akan mengcompile code ini dulu
  // And make sure to have your ganache network up!
  // make sure koneksi ganache sudah berjalan
  let provider = new ethers.providers.JsonRpcProvider(process.env.RPC_URL);
  let wallet = new ethers.Wallet(process.env.PRIVATE_KEY, provider);
  // const encryptedJson = fs.readFileSync("./.encryptedKey.json", "utf8");
  // let wallet = new ethers.Wallet.fromEncryptedJsonSync(
  //   encryptedJson,
  //   process.env.PRIVATE_KEY_PASSWORD
  // );
  // wallet = wallet.connect(provider);
  // variable abi yang akan membaca ABI dari smart contract kita
  const abi = fs.readFileSync("./SimpleStorage_sol_SimpleStorage.abi", "utf8");
  const binary = fs.readFileSync(
    "./SimpleStorage_sol_SimpleStorage.bin",
    "utf8"
  );
  // variable contract factory akan membaca abi, binary dan wallet
  const contractFactory = new ethers.ContractFactory(abi, binary, wallet);
  console.log("Deploying, please wait...");
  // variable contract yang akan menunggu beberapa waktu untuk contract di deploy
  const contract = await contractFactory.deploy();
  // const contract = await contractFactory.deploy({ gasPrice: 100000000000 })
  // variable deployementReceipt yang akan menunggu beberapa waktu untuk transaksi
  const deploymentReceipt = await contract.deployTransaction.wait(1);
  console.log(`Contract deployed to ${contract.address}`);
  // console.log("Here is the transaction:")
  // console.log(contract.deployTransaction)
  // console.log("Here is the receipt:")
  // console.log(transactionReceipt)
  // const nonce = await wallet.getTransactionCount()
  // tx = {
  //   nonce: nonce,
  //   gasPrice: 100000000000,
  //   gasLimit: 1000000,
  //   to: null,
  //   value: 0,
  //   data: "",
  //   chainId: 1337,

  // Additionally, there is a v,r,and s variable that ethers handles for us.
  // This is the signature of the transaction.
  // There is a lot of math going on with those values, but that's how it's gaurenteed that the transaction is signed!
  // https://ethereum.stackexchange.com/questions/15766/what-does-v-r-s-in-eth-gettransactionbyhash-mean

  // }
  // There is also a v, r, and s component of the transaction that Ethers will handle for us.
  // It's these three components that make up the cryptographic signature.
  // We won't go into this, because it's a lot of math.

  // console.log("Let's deploy another! Please wait...")
  // let resp = await wallet.signTransaction(tx)
  // const sentTxResponse = await wallet.sendTransaction(tx);
  // console.log(resp)

  // code untuk mendapatkan favorite number dan update nilai favorite number
  let currentFavoriteNumber = await contract.retrieve();
  console.log(`Current Favorite Number: ${currentFavoriteNumber}`);
  console.log("Updating favorite number...");
  let transactionResponse = await contract.store(7);
  let transactionReceipt = await transactionResponse.wait();
  currentFavoriteNumber = await contract.retrieve();
  console.log(`New Favorite Number: ${currentFavoriteNumber}`);
}
// code sinkron
main()
  .then(() => process.exit(0))
  .catch((error) => {
    console.error(error);
    process.exit(1);
  });

// synchronous [solidity]
// asynchronous [javascript]

// cooking
// Synchronous
// 1. Put popcorn in microwave -> Promise
// 2. Wait for popcorn to finish
// 3. Pour drinks for everyone

// // fungsi yang tidak harus sinkron
// async function setupMovieNight(){
//     await cookPopcorn()
//     await pourDrinks()
//     startMovie()
// }
// // fungsi yang harus sinkron
// function cookPopcorn(){
//     return Promise(pass)
// }
// // jadi ketika menjalankan fungsi cookpopcorn maka akan
// // mengembalikan nilai promise yang dimana promise itu akan
// // menjalankan fungsi cookpopcorn, pourdrink dan startmovie

// Asynchronous
// 1. Put popcorn in the mircrowave
// 2. Pour drinks for everyone
// 3. Wait for popcorn to finish

// Promise
// Pending
// Fulfilled
// Rejected
